...
if [[ ! -f cifar10-general-report.csv ]]; then
    echo "nb_shards, forgot_shard_id, accuracy, precision_macro, recall_macro, f1_macro, training_time" > cifar10-general-report.csv
fi

### Uncomment the case depending on the experiment you want to run.

### Inference without unlearning.
metric=$(python inference.py --model cifar10 --shards 10 --slices 3 --dataset datasets/cifar10/datasetfile --batch_size 64 --dropout_rate 0.2 --container "cifar10" | tail -n 1)
...
echo "10,None,${metric},${time}" >> cifar10-general-report.csv
